...
for i in range(0,totalpointclouds):
    #create txtfiles folder
    filename = './txtfiles/'+prefix+'pc'+str(i)+'.txt'
    array = np.zeros((w+1,d))
        
    for j in range(0,w):
        currindex = i*stride+j
        #slide by stride no. of frames
        array[j][0]=df.iat[currindex,1]
        array[j][1]=df.iat[currindex,2]
        array[j][2]=df.iat[currindex,3]
        array[j][3]=df.iat[currindex,4]
        array[j][4]=df.iat[currindex,5]
